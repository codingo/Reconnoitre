   ...
   for serv in serv_dict: 
       ...
       if ("ftp" in serv):
           ...
           for port in ports:
               ...
               f.write("      [=] gobuster -w /usr/share/wordlists/seclists/Discovery/Web_Content/common.txt -u http://%s:%s/\n -s '200,204,301,302,307,403,500' -e > %s/%s_gobuster_common.txt -t 50 \n" % (ip_address, port, outputdir, ip_address))
               f.write("      [=] gobuster -w /usr/share/wordlists/seclists/Discovery/Web_Content/cgis.txt -u http://%s:%s/\n -s '200,204,301,307,403,500' -e > %s/%s_gobuster_cgis.txt -t 50 \n" % (ip_address, port, outputdir, ip_address))
